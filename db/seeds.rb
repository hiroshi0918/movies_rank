...
# Fetch a reliable open-source list of 146 famous movies with real IMDB poster URLs
url = URI("https://raw.githubusercontent.com/erik-sytnyk/movies-list/master/db.json")
res = Net::HTTP.get(url)
movies_data = JSON.parse(res)["movies"]

# Dictionary to map famous movie titles to Japanese
ja_titles = {
...
# Only map up to 100 movies to fulfill request. Exclude un-reachable Amazon S3 images by replacing them if needed.
valid_movies = movies_data.reject { |m| m["posterUrl"].nil? }.uniq { |m| m["title"] }.take(100)
...
valid_movies.each_with_index do |data, index|
  ...
  # Fallback for known broken image URLs from the dataset
  fallback_images = {
  ...
  image_url = fallback_images[original_title] if fallback_images.key?(original_title)
